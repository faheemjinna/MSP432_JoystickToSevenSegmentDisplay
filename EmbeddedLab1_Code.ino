...
void loop()
{
    ...
    //Instead of 0-9 we have pushed the limit to 0-99 and analogRead is set to Ypin.
    y = map(analogRead(joystickYPin), 0, 4000, 0, 99);

    //Also, the contrain of the value y has been updated to 99.
    y = constrain(y, 0, 99);
  
    //Printing the value to the Serial_monitor before seperating it.
    Serial.print("Current value is: ");
    Serial.println(y);

    //Seperating the y value in ones & tens.
    int x;
    x = y%10;
    y /= 10;

    //Setting the seperated value to the display.
    setOnes(x);
    setTens(y);

    //Setting a delay of 100 to see the change more precisely.
    delay(100);
}
